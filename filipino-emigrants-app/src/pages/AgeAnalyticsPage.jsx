...
const AgeAnalyticsPage = ({ rawData }) => {
  const [chartFilters, setChartFilters] = useState({
    ...
    pieChart: { year: 'all' },
    lineChart: { 
      ageGroup: '14-below', // Default set to 14-below
      yearRange: [1981, 2020] 
    }
  });
  ...
  // Bar Chart Data - only depends on bar chart filters
  ...
  // Pie Chart Data - only depends on pie chart filters
  const { pieChartData, hasPieData } = useMemo(() => {
    let filteredData = [];
    ...
    if (!filteredData.length) return { pieChartData: [], hasPieData: false };
  ...
  // Individual chart render functions with Legends
    ...
    <ResponsiveContainer width="100%" height={isFullScreen ? "90%" : "100%"}>
      <PieChart>
        <Pie
          ...
          cy="50%"
          labelLine={false}
          label={({ ageGroup, percent }) => `${ageGroup}: ${(percent * 100).toFixed(1)}%`}
    ...
    <div className="data-page age-analytics-page">
      ...
      {fullScreenChart && (
        ...
        >
          ...
          {fullScreenChart === "Age Group Composition" && renderFullScreenPieChart()}
          {fullScreenChart === "Age Group Trend" && (
      ...
      {hasAnyData ? (
        <div className={`charts-grid ${fullScreenChart ? 'blurred' : ''}`}>
          ...
          <div className="charts-top-row">
            {/* Bar Chart – Age Distribution (Single Year) */}
            <ChartContainer 
            ...
            {/* Pie Chart – Composition of Age Groups */}
            <ChartContainer 
          ...
          {/* Bottom Row: Single centered large chart with proper spacing */}
          <div className="charts-bottom-row">
            {/* Line Chart – Age Group Trend */}
            <ChartContainer 
              title="Age Group Trend" 
              icon={<FiTrendingUp />}
              className="centered-trend-chart"
              filters={
                <div className="chart-filters-row">
                  <AgeGroupDropdownFilter 
                    value={chartFilters.lineChart.ageGroup}
