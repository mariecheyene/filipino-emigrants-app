...
// Add to src/utils/dataPreparation.js

/**
 * Prepare sex data specifically for forecasting
 */
export function prepareSexData(sexData, selectedGender = 'total') {
  if (!sexData || sexData.length === 0) return [];
  
  return sexData.map(item => ({
    year: item.year,
    // Use the selected gender attribute
    value: item[selectedGender] || item.total || 0,
    // Keep all data for potential multi-feature models
    male: item.male || 0,
    female: item.female || 0,
    total: item.total || 0,
    // Optional: create derived features
    maleRatio: item.total ? (item.male / item.total) : 0,
    femaleRatio: item.total ? (item.female / item.total) : 0
  }));
}

/**
 * Create sequences specifically for sex data forecasting
 */
export function createSexSequences(data, lookback = 3, features = ['value'], target = 'value') {
  const X = [];
  const y = [];
  ...
  for (let i = lookback; i < data.length; i++) {
    ...
    // Target is the next year's value
    y.push(data[i][target]);
  }
